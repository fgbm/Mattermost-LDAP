...
//echo "Start connect ".$hostname."  ".$port."<br>";
...
//echo "Start check Login <br>";
        ...
        if ($bind_dn != '' && $bind_dn != null) {
            //echo 'st bind',$bind_dn,' : ',$bind_pass,'<br/>';
        ...
        //echo 'st2<br/>';
        error_log("ldap search: " . $base_dn . " " . $search_filter, 0);
        $result = ldap_search($this->ldap_server, $base_dn, $search_filter, array(), 0, 1, 500);
        //echo 'st3:  base_dn="',$base_dn,'" , filter=',$search_filter,'<br/>';
        if (!$result) {
            //echo 'No result<br/>';
    ...
    public function getDataForMattermost($base_dn, $filter, $bind_dn, $bind_pass, $search_attribute, $user)
    {
        ...
        //    	$attribute=array("cn","userPrincipalName",);
        ...
        //error_log("search filter: ".$base_dn." ".$search_filter, 0);
        $result = ldap_search($this->ldap_server, $base_dn, $search_filter, array(), 0, 1, 500);

        if (!$result) {
            throw new Exception('An error has occurred during ldap_search execution. Please check parameter of LDAP/getData.');
        }

        $data = ldap_first_entry($this->ldap_server, $result);
        if (!$data) {
            throw new Exception('An error has occurred during ldap_first_entry execution. Please check parameter of LDAP/getData.');
        }

        $mail = ldap_get_values($this->ldap_server, $data, "userPrincipalName");
        if (!$mail) {
            throw new Exception('An error has occurred during ldap_get_values execution (userPrincipalName). Please check parameter of LDAP/getData.');
        }

        $usermail = $mail[0];
        $realmail = ldap_get_values($this->ldap_server, $data, "mail");
        if ($realmail) {
            if (filter_var($realmail[0], FILTER_VALIDATE_EMAIL)) {
                $usermail = $realmail[0];
            }
        }
        ...
        if (!$cn) {
            throw new Exception('An error has occurred during ldap_get_values execution (complete name). Please check parameter of LDAP/getData.');
        }

        $description = ldap_get_values($this->ldap_server, $data, "description");
        $phone = ldap_get_values($this->ldap_server, $data, "telephoneNumber");
        $post = "";
        $sn = ldap_get_values($this->ldap_server, $data, "sn");
        $givenName = ldap_get_values($this->ldap_server, $data, "givenName");
